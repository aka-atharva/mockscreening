...
export function ActivityTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* User Activity Summary with inline styles */}
      <div className="bg-card p-4 rounded-lg border border-border mb-4">
        <h3 className="text-lg font-semibold mb-3">User Activity Summary</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Object.entries(summary).map(([username, data]) => (
            <div key={username} className="p-3 bg-background border border-border rounded-lg">
              <div className="flex items-center mb-2">
                <User className="h-4 w-4 mr-2 text-violet-500" />
                <span className="font-medium text-foreground">{username}</span>
              </div>
              <div className="text-sm text-muted-foreground">
                <div>Total Actions: {data.totalActions}</div>
                <div>Page Visits: {data.pageVisits}</div>
                <div>Logins: {data.loginCount}</div>
                <div>Last Active: {formatDisplayDate(data.lastActive)}</div>
              </div>
            </div>
          ))}
        </div>
